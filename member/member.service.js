...
async function getAll() {
    return await Member.find().sort({"currentExp" : 1});//.select('-hash');
}
...
async function addExp(params){
    ...
    console.log("current level after lvl up: " + currentLvl + "new level: " + member.currentLevel);
    member.save();
...
function getLevelXPNeeded(desired_level){
    return 5 / 6 * desired_level * (2 * desired_level * desired_level + 27 * desired_level + 91);;
}
...
async function update(id, memberParam) {
    ...
    if (member.membername !== memberParam.membername && await Member.findOne({ membername: memberParam.membername })) {
        throw 'Membername "' + memberParam.membername + '" is already taken';
    }
